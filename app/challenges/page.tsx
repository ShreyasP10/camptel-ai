...
const categoryColors: Record<string, string> = {
  academic: "border-l-brand-500 bg-brand-50",
  placement: "border-l-emerald-500 bg-emerald-50",
  infrastructure: "border-l-amber-500 bg-amber-50",
};
...
function ChallengeDetail({
  ...
  const [currentQuestion, setCurrentQuestion] = useState(challenge.question);
  ...
  const color = categoryColors[challenge.category] || "border-l-surface-300 bg-surface-50";
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 text-sm font-medium text-surface-500 hover:text-surface-700 transition"
      >
      ...
      <div className={`rounded-2xl border-l-4 p-6 ${color}`}>
        ...
        <h2 className="mt-3 text-xl font-bold text-surface-900">{challenge.title}</h2>
        <p className="mt-1 text-surface-600">{challenge.description}</p>
      </div>
      ...
      {answer && (
        <>
          <Card variant="bordered" className="border-brand-200 bg-gradient-to-br from-brand-50 to-white">
            <div className="flex items-start gap-3">
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 text-sm">
                ✦
              </span>
              <div>
                <p className="text-sm font-semibold text-brand-700">AI Insight</p>
                <p className="mt-2 text-surface-700 leading-relaxed">{answer}</p>
              </div>
            </div>
          </Card>
          ...
          {sql && (
            <details className="rounded-xl border border-surface-200 bg-surface-50 p-4">
              <summary className="cursor-pointer text-sm font-semibold text-surface-600 hover:text-surface-800">
                Show SQL Query
              </summary>
              <pre className="mt-3 overflow-x-auto rounded-lg bg-surface-900 p-4 text-xs text-emerald-400 font-mono whitespace-pre-wrap">
                {sql}
              </pre>
            </details>
          )}
            ...
            <Card title="Results" subtitle={`${rows.length} row${rows.length !== 1 ? "s" : ""} returned`}>
              <div className="overflow-x-auto scrollbar-thin">
                <table className="min-w-full text-left text-sm">
                  <thead>
                    <tr className="border-b border-surface-100 text-surface-400">
                      {Object.keys(rows[0]).map((key) => (
                        <th key={key} className="pb-3 pr-4 font-semibold capitalize">
                          {key.replace(/_/g, " ")}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {rows.map((row, i) => (
                      <tr key={i} className="border-b border-surface-50 transition hover:bg-surface-50">
                        {Object.values(row).map((val, j) => (
                          <td key={j} className="py-3 pr-4 text-surface-600">
                            {String(val ?? "—")}
                          </td>
                        ))}
                      </tr>
            ...
            <div className="flex flex-wrap gap-2">
              {challenge.followUp.map((q) => (
                <button
                  ...
                  disabled={loading}
                  className="rounded-xl border border-surface-200 bg-white px-4 py-2.5 text-sm font-medium text-surface-600 shadow-sm transition hover:border-brand-200 hover:text-brand-600 disabled:opacity-50"
                >
                  {q}
                </button>
              ))}
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {(challenges as Challenge[]).map((challenge) => {
          ...
          return (
            ...
            >
              <span className="text-2xl">{categoryIcons[challenge.category] || "📋"}</span>
              <h3 className="mt-3 text-lg font-semibold text-surface-900 group-hover:text-brand-700 transition">
                {challenge.title}
              </h3>
              <p className="mt-1.5 text-sm text-surface-500 leading-relaxed">
                {challenge.description}
              </p>
              <span className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-brand-600 opacity-0 group-hover:opacity-100 transition">
                Run challenge
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
